...
Avant CAST, contrôler `collection-report.json`, `unresolved-paths.txt`, la couverture à 100 %, `baseline-diff.json` et `security/sanitization-report.json`.

## Première mise en service

1. Exécuter un pilote App A/cible QNX.
...
5. Répéter sur App B puis sur chaque cible réellement analysée.
6. Geler la version, le schéma et la baseline.

## Incident et rollback

- conserver le bundle d’entrée, le package, le manifeste et les logs ;
